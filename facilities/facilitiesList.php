...
<div class="contain">
  ...
  <div class="listr right">
    <h2><span>您现在的位置: <a href="/">山东省钢结构协会</a>>><a href="facilitiesList.php">制造基地</a></span>制造基地</h2>
    <ul>

      <?php
        require_once '../manage/model/facilities.class.php';
        $facilities = new facilities();
        $allfacilities = $facilities->selectAllfacilities();
        if ($allfacilities!=null) {
            ...
            if ($pagefacilities!=null) {
                for ($i=0; $i < count($pagefacilities); $i++) { 
                    $content = "<li><a href='facilitiesDetail.php?facilitiesId=".$pagefacilities[$i]['facilitiesId']."'><span>" . $pagefacilities[$i]['time'] ."</span>".$pagefacilities[$i]['title']." </a></li>";
                    echo $content;
                }
            }else{
