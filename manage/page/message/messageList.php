                        ...
                        <div class="card-header bg-light">
	                        <div class="row">
		                        <div class="col-md-4">
		                     	...
		                     	<div class="col-md-6">
		                            <button type="button" onclick="javascript:location.href='messageAdd.php'" class="btn btn-primary">
...
<!-- 	                                <button type="button" id="delBtn" class="btn btn-danger">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped text-center">
                                    ...
                                    <tbody>
                                        <?php
                                            require '../../model/Message.class.php';
                                            $Message = new Message();
                                            if (isset($_REQUEST['keyword'])&&$_REQUEST['keyword']!="") {
                                                $allMessage = $Message->selectMessageByKeyword($_REQUEST['keyword']);
                                            }else{
                                                $allMessage = $Message->selectAllMessages();
                                            }
                                            
                                            if ($allMessage!=null) {
                                                $totalNum = count($allMessage); //数据总条数
                                                $pageSize = 8;
                                                ...
                                                if (isset($_REQUEST['keyword'])&&$_REQUEST['keyword']!="") {
                                                    $pageMessage = $Message->selectMessageByPageKeyword($offset, $pageSize, $_REQUEST['keyword']);
                                                }else{
                                                    $pageMessage = $Message->selectMessageByPage($offset, $pageSize);
                                                }

                                                if ($pageMessage!=null) {
                                                    for ($i=0; $i < count($pageMessage); $i++) { 
                                                        $content = "<tr><td>" . $pageMessage[$i]['messageTitle'] . "</td>";
                                                        $content .= "<td class='text-nowrap'>" . $pageMessage[$i]['messageTime'] ."</td>";
                                                        $content .= "<td>". 
                                                        "<a href='messageDetail.php?messageId=".$pageMessage[$i]['messageId']."'><i class='fa fa-eye'></i>&nbsp;查看内容&nbsp;&nbsp;&nbsp;</a>".  
                                                        "<a href='messageEdit.php?messageId=".$pageMessage[$i]['messageId']."'><i class='fa fa-wrench'></i> &nbsp;修改&nbsp;&nbsp;&nbsp; </a>".    
                                                        "<a href=\"javascript:if(confirm('确实要删除吗?'))location='../../dao/messageDao.php?mode=del&messageId=".$pageMessage[$i]['messageId']."'\"><i class='fa fa-trash'></i> &nbsp;删除</a>" . 
                                                        "</td>";
                                                        $content .= "</tr>";
                                                        echo $content;
                                                    }
